...
import { Logger, confirmAndMarkAsRead, isDifferentChannel, isMyMessage, useForceUpdate, useFreshCallback, useUniqHandlerId } from '@sendbird/uikit-utils';
...
export const useGroupChannelMessagesWithCollection = (sdk, channel, userId, options) => {
  ...
  const init = useFreshCallback(async (startingPoint, limit, callback) => {
    ...
    collectionRef.current.initialize(MessageCollectionInitPolicy.CACHE_AND_REPLACE_BY_API).onCacheResult((err, messages) => {
      if (err) sdk.isCacheEnabled && Logger.error('[useGroupChannelMessagesWithCollection/onCacheResult]', err);else {
      ...
      if (err) Logger.warn('[useGroupChannelMessagesWithCollection/onApiResult]', err);else {
  ...
  const hasNext = useFreshCallback(() => {
    if (collectionInitializedRef.current && collectionRef.current) {
  ...
  const resetWithStartingPoint = useFreshCallback((startingPoint, callback) => {
    updateLoading(true);

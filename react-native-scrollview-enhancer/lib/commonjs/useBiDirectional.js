...
exports.useBiDirectional = useBiDirectional;
...
function useBiDirectional(Component, props, ref) {
  const innerRef = (0, _react.useRef)();
  const scrollRef = () => ref || innerRef;
  const scrollMetrics = (0, _react.useRef)({
    offset: 0,
    visibleLength: -1,
    contentLength: -1,
    timestamp: 0
  });
  const inProgressCall = (0, _react.useRef)(Promise.resolve());
  const isHorizontal = props.horizontal ?? false;
  const sentEndForContentLength = (0, _react.useRef)(new Map());
  const sentStartForContentLength = (0, _react.useRef)(new Map());

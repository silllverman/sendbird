...
const SendInput = /*#__PURE__*/forwardRef(function SendInput(_ref, ref) {
    ...
    openSheet({
      sheetItems: [{
        ...
        onPress: async () => {
          ...
          if (mediaFile) {
            // Image compression
            if (isImage(mediaFile.uri, mediaFile.type) && shouldCompressImage(mediaFile.type, features.imageCompressionEnabled)) {
        ...
        onPress: async () => {
          ...
          if (documentFile) {
            // Image compression
            if (isImage(documentFile.uri, documentFile.type) && shouldCompressImage(documentFile.type, features.imageCompressionEnabled)) {
              await SBUUtils.safeRun(async () => {
                const compressed = await mediaService.compressImage({
  ...
  }, mentionManager.textToMentionedComponents(text, mentionedUsers)), Boolean(text.trim()) && /*#__PURE__*/React.createElement(TouchableOpacity, {
